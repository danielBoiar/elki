...
class Solver {
	...
	double eps;
	double Cp,Cn;
	double[] p; //zero vector for OCSVM
	int[] active_set;
	double[] G_bar;		// gradient, if we treat free variables as 0
	int l;
	boolean unshrink;	// XXX
	int t; // t-th example is left out for leave one out
	...
	Solver(QMatrix Q, double[] QD){//not used yet
		this.Q = Q;
		this.QD = QD;
	}

	Solver(double[] G, double[] G_bar){//Constructor for Given G and G_bar
		this.G = G;
		this.G_bar = G_bar; 
	}
	...
	void reconstruct_gradient()
	{
		...
		if(active_size == l) return;//evtl ist diese zeile problematisch und verhindert die rekonstruktion
	...
	void SolveGivenG(int l, QMatrix Q, double[] p_, byte[] y_,
			   double[] alpha_, double Cp, double Cn, double eps, SolutionInfo si, int shrinking)
	{
		this.l = l;
		this.Q = Q;
	...
	int select_working_set(int[] working_set)
	{
		...
		if(Gmax+Gmax2 < eps || Gmin_idx == -1){
			svm.info("\n WorkingSet already optimal \n");
			return 1;
		}
		working_set[0] = Gmax_idx;
		working_set[1] = Gmin_idx;
		svm.info("WorkingSet: i=" +working_set[0]+" , j="+ working_set[1]+" \n");
		return 0;
	}
...
public class svm {
	...
	ONE_CLASS_Q cacheOneClassSVM;
	double[] G;
	double[] G_bar;

	public svm(svm_problem prob, svm_parameter param){
		cacheOneClassSVM = new ONE_CLASS_Q(prob, param);
	}

	public void swapCache(int i, int j){
		cacheOneClassSVM.swap_index(i,j);
	}
	public float[] getQ(int i, int l){ return cacheOneClassSVM.get_Q(i,l); }

	public double[] getG() {return G;}
	public void setG(double[] g) {G = g;}
	public double[] getG_bar() {return G_bar;}
	public void setG_bar(double[] g_bar) {G_bar = g_bar;}
      ...
      double[] alpha, Solver.SolutionInfo si)
  {
    ...
    info("R^2 = "+r_square+"\n");
    if(C > 1 && param.svm_type == svm_parameter.SVDD)
      ...
      double[] alpha, Solver.SolutionInfo si, svm_model model_t, ONE_CLASS_Q cache)
  {
    ...
    {
      QD[i] = cache.get_QD()[i];
      linear_term[i] = -0.5 * cache.get_QD()[i];
    }
    ...
    if(C > (double)1/l)
    {
      ...
      s.SolveGivenG(l, cache, linear_term, ones,
      ...
      double[] alpha, Solver.SolutionInfo si, ONE_CLASS_Q cache)
  {
    ...
    if(C > (double)1/l)
    {
      ...
      Solver s = new Solver();
      s.Solve(l, cache, linear_term, ones, alpha, C, C,
          param.eps, si, param.shrinking);
      G = s.getG().clone();
      G_bar = s.getG_bar().clone();
      ...
      case svm_parameter.SVDD:
		...
		for(int i=0;i<prob.l;i++)
		{
			if(Math.abs(alpha[i]) > 0)
			{
				...
				for(int dimension = 0; dimension < dim - 1; dimension++){
					xValuesString += prob.x[i][dimension].value+",";
				}
				xValuesString += prob.x[i][dim - 1].value;
				String svString = i+","+alpha[i]+","+xValuesString;
				svm.info(svString);
				if(prob.y[i] > 0)
				{
	...
	decision_function svm_train_oneGivenAlphaAndCache(
			svm_problem prob, svm_parameter param,
			double Cp, double Cn, svm_model model_t)
	{
		...
		switch(param.svm_type)
		{
			...
			case svm_parameter.NU_SVC:
				...
				break;
			case svm_parameter.ONE_CLASS:
				solve_one_classGivenAlphaAndCache(prob,param,alpha,si, model_t, cacheOneClassSVM);
				break;
			case svm_parameter.EPSILON_SVR:
				solve_epsilon_svr(prob,param,alpha,si);
				break;
			case svm_parameter.NU_SVR:
				...
				break;
			case svm_parameter.SVDD:
        solve_svddGivenAlphaAndCache(prob,param,alpha,si, model_t, cacheOneClassSVM);
        break;
		}
			...
			double Cp, double Cn, svm_model model_t)
	{
		...
		switch(param.svm_type)
		{
			...
			case svm_parameter.NU_SVC:
				...
				break;
			case svm_parameter.ONE_CLASS:
				solve_one_classGivenAlpha(prob,param,alpha,si, model_t);
				break;
			case svm_parameter.EPSILON_SVR:
				solve_epsilon_svr(prob,param,alpha,si);
				break;
		...
		int nSV = 0;
		int nBSV = 0;
		svm.info("index,alpha,x,y");
		for(int i=0;i<prob.l;i++)
		{
			if(Math.abs(alpha[i]) > 0)
			{
				++nSV;
				svm.info(i+","+alpha[i]+","+prob.x[i][0].value+","+ prob.x[i][1].value);
				if(prob.y[i] > 0)
				{
					if(Math.abs(alpha[i]) >= si.upper_bound_p)
						++nBSV;
			...
			double Cp, double Cn, ONE_CLASS_Q cache)
	{
		...
		switch(param.svm_type)
		{
			...
			case svm_parameter.NU_SVC:
				...
				break;
			case svm_parameter.ONE_CLASS:
				solve_one_classGivenCache(prob,param,alpha,si,cache);
				break;
			case svm_parameter.EPSILON_SVR:
				solve_epsilon_svr(prob,param,alpha,si);
				break;
			case svm_parameter.NU_SVR:
				...
				break;
			case svm_parameter.SVDD:
        solve_svddGivenCache(prob,param,alpha,si,cache);
        break;
		}
   ...
   */
	public svm_model svm_trainGivenCache(svm_problem prob, svm_parameter param)
	{
				...
				param.svm_type == svm_parameter.NU_SVR)
		{
			...
			decision_function f = svm_train_oneGivenCache(prob,param,0,0,cacheOneClassSVM);
			model.rho = new double[1];
			model.rho[0] = f.rho;
   ...
   */
	public static svm_model svm_trainGivenAlpha(svm_problem prob, svm_parameter param, svm_model model_t)
	{
				...
				param.svm_type == svm_parameter.NU_SVR)
		{
			...
			decision_function f = svm_train_oneGivenAlpha(prob,param,0,0,model_t);
			model.rho = new double[1];
			model.rho[0] = f.rho;
   ...
   */
	public svm_model svm_trainGivenAlphaAndCache(svm_problem prob, svm_parameter param, svm_model model_t)
	{
				...
				param.svm_type == svm_parameter.NU_SVR)
		{
			...
			decision_function f = svm_train_oneGivenAlphaAndCache(prob,param,0,0,model_t);
			model.rho = new double[1];
			model.rho[0] = f.rho;
   ...
   */
	public static double svm_predict_values(svm_model model, svm_node[] x, double[] dec_values)
	{
  	...
  	else if(model.param.svm_type == svm_parameter.SVDD )
  	{
	...
	public static double svm_predict(svm_model model, svm_node[] x)
	{
		...
		if(model.param.svm_type == svm_parameter.ONE_CLASS ||
				model.param.svm_type == svm_parameter.EPSILON_SVR ||
				model.param.svm_type == svm_parameter.NU_SVR ||
				model.param.svm_type == svm_parameter.SVDD)
	...
	{
		"c_svc","nu_svc","one_class","epsilon_svr","nu_svr","svdd"
	};
